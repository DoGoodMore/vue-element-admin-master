 ...
 *  下标菜单只会在其子菜单多余一个的时候才会显示 即如果只有一个菜单 那么就不会显示下拉菜单的按钮
**/

/**
* hidden: true                   if `hidden:true` will not show in the sidebar(default is false)
* 当在每个route 中带入参数hidden: true时  那么该路由所对应的菜单不会显示在侧边栏中
*                                if not set alwaysShow, only more than one route under the children
*                                it will becomes nested mode, otherwise not show the root menu
*                                如果设置了alwaysShow 那么不管是否有子菜单这个菜单总会显示
* redirect: noredirect           if `redirect:noredirect` will no redirct in the breadcrumb
* 如果`redirect：noredirect`不会在面包屑中重定向
* name:'router-name'             the name is used by <keep-alive> (must set!!!)
* 必须在传入的每个route中设置对应的name属性
* meta : {
    roles: ['admin','editor']     will control the page roles (you can set multiple roles)
    roles: 用来设定能访问当前路由组件的对应所有角色
    title: 'title'               the name show in submenu and breadcrumb (recommend set)
    title 通过设置其title属性 在子菜单或是面包屑中的名称中显示
    icon: 'svg-name'             the icon show in the sidebar,
    icon: 用来设置在显示菜单时候的icon的类名
    noCache: true                if true ,the page will no be cached(default is false)
    noCache: 是否禁用缓存 如果禁用缓存那么设置的默认选项将会失效
  }
**/
//注册路由地址并动态引入对应的组件
...
//定义并暴露了后续需要权限验证的所有菜单 通过权限验证后动态添加到对应的vue-routes中
